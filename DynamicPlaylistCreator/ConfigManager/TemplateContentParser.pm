...
sub loadTemplate {
	...
	for my $plugindir (@pluginDirs) {
		if (-d catdir($plugindir, "DynamicPlaylistCreator", "Templates")) {
			my @subDirs = ('Songs', 'Artists', 'Albums', 'Genres', 'Years', 'Playlists', 'HomeExtras');
			splice @subDirs, 3, 0, 'Works' if (Slim::Utils::Versions->compareVersions($::VERSION, '9.0') >= 0);
			push @subDirs, 'HomeExtras' if Slim::Utils::PluginManager->isEnabled('Plugins::MaterialSkin::Plugin') && Plugins::MaterialSkin::Plugin->can('registerHomeExtra');
			foreach (@subDirs) {
