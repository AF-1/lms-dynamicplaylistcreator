...
sub parseTemplateContent {
	...
	if ($content) {
		...
		if ($@) {
			...
			# is dpl is context menu list?
			my $isContextMenuList = $template->{'contextmenu'};
			$templateParameters{'contextmenu'} = $isContextMenuList;

			# is dpl is home extras menu?
			my $isHomeExtraMenu = $template->{'homeextrasmenu'};
			$templateParameters{'homeextrasmenu'} = $isHomeExtraMenu;
			...
			if (defined($template->{'parameter'})) {
				...
				$localcontext->{'homeextrasmenu'} = $templateParameters{'homeextrasmenu'} || 0;
			...
			my %playlist = (
				...
				'homeextrasmenu' => $localcontext->{'homeextrasmenu'},
